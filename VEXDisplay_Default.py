...
class TeamPanel(wx.Panel):
    def __init__(self,parent,id,team=None,size=(0,0),pos=(0,0),bg=Colors["White"],fileStr = None):
        ...
        if team != None:
            ...
            self.Rank.SetPosition((75-(self.Rank.GetSize()[0]/2),-10))
    ...
    def UpdateRankPositions(self):
        for panel in range(0,len(self.RankPanels)):
            if self.RankPanels[panel].GetPosition()[1] < (-1*self.RankPanels[panel].GetSize()[1]):#Checks to see if panel is off screen
                    newY = panel*self.rankHeight + self.RankPanels[len(self.RankPanels)-1].GetPosition()[1] + self.RankPanels[len(self.RankPanels)-1].GetSize()[1]-3#if it is reset the y
                ...
                self.RankPanels[panel].SetPosition((self.RankPanels[panel].GetPosition()[0],self.RankPanels[panel].GetPosition()[1]-3))
        if self.RankPanels[len(self.RankPanels)-1].GetPosition()[1] == 852:#1066 represents the y-coordinate at which the panel will be visible
            self.UpdateRankData()
        
        wx.CallLater(33,self.UpdateRankPositions)
